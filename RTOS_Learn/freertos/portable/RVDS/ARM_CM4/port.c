...
__asm void prvStartFirstTask( void )
{
    ...
    /* 调用SVC去启动第一个任务 */
	svc 0  		// 产生一个SVC_Handler中断
	nop
	nop
}

// prvStartFirstTask函数的svc 0代码就会跳转到这。FreeRTOSConfig.h文件中定义的，会将#define vPortSVCHandler SVC_Handler
__asm void vPortSVCHandler( void )
{
	...
	ldr	r3, =pxCurrentTCB			/* pxCurrentTCB是任务控制块地址 */
	ldr r1, [r3]							/* 现在r1就是任务控制块空间的第一块地址了，也就是pxTopOfStack栈顶指针	*/
	ldr r0, [r1]							/* 目前r0的值等于第一个任务堆栈的栈顶 */
	...
	bx r14                  	/* 异常返回，这个时候栈中的剩下内容将会自动加载到CPU寄存器：
                               xPSR，PC（任务入口地址），R14，R12，R3，R2，R1，R0（任务的形参）
															 没错，freertos就是只允许一个形参放在R0，如果很多就要用结构体传参进去
                               同时PSP的值也将更新，即指向任务栈的栈顶 */
}

// 切换任务，保存上一个任务的上下文，然后切换到下一个任务
__asm void xPortPendSVHandler( void )
{
	...
	/* 获取任务栈指针到r0 */
	mrs r0, psp										// psp是当前任务的栈
	isb

	ldr	r3, =pxCurrentTCB					/* 加载pxCurrentTCB的地址到r3 */
	ldr	r2, [r3]                	/* 加载pxCurrentTCB到r2，现在R2就是栈顶指针，还不是栈顶 */

	stmdb r0!, {r4-r11}						/* 将CPU寄存器r4~r11的值存储到r0指向的地址，r0!会自动修改地址 */
