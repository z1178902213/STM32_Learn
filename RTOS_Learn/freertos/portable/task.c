...
static void prvResetNextTaskUnblockTime( void );                 /* 任务控制块指针 */
...
// 这个函数没有做多少事情，怀疑可能是为了兼容或者解耦做的，可能是和动态创建任务做区分。
											...
											)
{
	...
	{
		// 把任务控制块先初始化了，然后就调用prvInitialiseNewTask去初始化任务。
									...
									)
{
	...
	/* 获取栈顶地址，pxStack是指向栈起始地址，由于入栈是由高到低，因此要把栈顶指针指向栈顶位置 */
	pxTopOfStack = pxNewTCB->pxStack + ( ulStackDepth - ( StackType_t ) 1 );
	
	//pxTopOfStack = ( StackType_t * ) ( ( ( portPOINTER_SIZE_TYPE ) pxTopOfStack ) & ( ~( ( portPOINTER_SIZE_TYPE ) portBYTE_ALIGNMENT_MASK ) ) );
	/* 向下做8字节对齐，对齐只会向低位移动栈顶指针，最多会丢弃栈顶2个字的空间 */
	...
	// xStateListItem是一个链表的节点，可以被挂到任意一个链表中。如果挂到阻塞链表，就表示在阻塞链表中排队了
...
static void prvAddNewTaskToReadyList( TCB_t *pxNewTCB )
{
	...
	taskENTER_CRITICAL();
	{
		...
		{
				// ？？这里不太明白，为什么这么着急就把currentTCB指向了最高优先级的TCB？？不得等下一个tick的时候去判断然后切换吗？
				// 好像默认就是在启动调度之前就要把任务全部都创建好？中途不知道能不能再创建任务。要是可以中途创建岂不是乱了？
...
/* 初始化任务就绪列表的链表根节点，每一个就绪列表数组都是一个优先级的链表头 */
