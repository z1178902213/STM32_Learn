...
static TaskHandle_t Test_Uart_Task_Handle = NULL;
...
static void Test_Uart1_Task(void);
...
//	}
	...
	xReturn = xTaskCreate(
							(TaskFunction_t) Test_Uart1_Task,
							(const char*) "Test_Uart1_Task",
							(uint32_t) configMINIMAL_STACK_SIZE,
							NULL,
							(UBaseType_t) 2,
							(TaskHandle_t *) &Test_Uart_Task_Handle);
